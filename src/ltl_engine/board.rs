...
impl Board {
    ...
    pub fn new(size: u64, config: Config) -> Self {
        Board {
            config,
            cells: Box::new(vec![vec![0; size as usize]; size as usize]),
        }
    }
    ...
    pub fn update(&mut self) -> () {
        ...
        let mut updated_board = Box::new(vec![vec![0; board_length]; board_length]);
        for x in 0..board_length {
            for y in 0..board_length {
                updated_board[x][y] = self.update_cell(x, y);
            }
        }
        self.cells = updated_board;
    }
    ...
    ///
    fn update_cell(&mut self, x: usize, y: usize) -> Cell {
...
mod tests {
    ...
    fn test_cells() -> Box<Cells> {
        Box::new(vec![vec![0, 1, 2], vec![3, 4, 5], vec![6, 7, 8]])
    }
    ...
    fn get_neighbourhood_moore_bigger_radius() {
        ...
        let mut i = 0;
        let mut cells = Box::new(vec![vec![0; 7]; 7]);
        for x in 0..7 {
            for y in 0..7 {
                cells[x][y] = i;
                i += 1;
    ...
    fn get_neighbourhood_count_moore_with_center() {
        ...
        let board = Board {
            config,
            cells: Box::new(vec![vec![0, 0, 0], vec![1, 2, 3], vec![4, 0, 0]]),
        };
    ...
    fn get_neighbourhood_count_moore_full() {
        ...
        let board = Board {
            config,
            cells: Box::new(vec![vec![1, 2, 3], vec![4, 5, 6], vec![7, 8, 9]]),
        };
        let neighbourhood_count = board.get_neighbourhood_count(1, 1);
    ...
    fn get_neighbourhood_count_neumann_with_center() {
        ...
        let board = Board {
            config,
            cells: Box::new(vec![vec![0, 5, 0], vec![1, 2, 3], vec![4, 0, 0]]),
        };
        let neighbourhood_count = board.get_neighbourhood_count(1, 1);
        assert_eq!(neighbourhood_count, 4);
    }
    ...
    fn update_all_alive() {
        ...
        board.cells = Box::new(vec![vec![1; 10]; 10]);
    ...
    fn update_all_dead() {
        ...
        let mut board: Board = Board::new(10, config);
        board.cells = Box::new(vec![vec![2; 10]; 10]);
    ...
    fn update_stick_repeat() {
        ...
        board.cells = Box::new(stick_vertical.clone());
        board.update();
